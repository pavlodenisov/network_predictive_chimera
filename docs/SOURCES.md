...
| `WebSearchSource` | **working (disabled without a key)** | live queries against the Tavily search API (https://tavily.com); set `TAVILY_API_KEY` to enable. `update_known_entities`: one query per monitored person → real news/funding/role-change mentions. `discover`: one `site:linkedin.com/in` query per `DiscoveryRule` with `titles` set — never contacts linkedin.com, only reads what Tavily's own index already crawled (same as a Google result card); a candidate's name is parsed from the search index's title metadata and discarded (not guessed) if it doesn't cleanly match a name shape (§5, §39, CLAUDE.md #11) | news articles for known people; named LinkedIn-indexed discovery candidates |
